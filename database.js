...
if (!custCols.includes('whatsapp_consent')) {
db.exec(`ALTER TABLE customers ADD COLUMN whatsapp_consent INTEGER DEFAULT 0`);
}
...
createCustomer: db.prepare(`INSERT INTO customers (name,company,phone,email,address,notes,payment_frequency,frequency_value,payment_amount,next_due_date,whatsapp_consent) VALUES (?,?,?,?,?,?,?,?,?,?,?)`),
updateCustomer: db.prepare(`UPDATE customers SET name=?,company=?,phone=?,email=?,address=?,notes=?,payment_frequency=?,frequency_value=?,payment_amount=?,next_due_date=?,whatsapp_consent=?,updated_at=datetime('now') WHERE id=?`),
...
getAllInvoices: db.prepare(`SELECT i.*, c.name as customer_name, c.phone as customer_phone, c.whatsapp_consent FROM invoices i JOIN customers c ON i.customer_id = c.id ORDER BY i.created_at DESC`),
getInvoiceById: db.prepare(`SELECT i.*, c.name as customer_name, c.phone as customer_phone, c.email as customer_email, c.company as customer_company, c.whatsapp_consent FROM invoices i JOIN customers c ON i.customer_id = c.id WHERE i.id = ?`),
createInvoice: db.prepare(`INSERT INTO invoices (invoice_id, customer_id, content, amount, invoice_date, due_date, status) VALUES (?, ?, ?, ?, ?, ?, ?)`),
updateInvoice: db.prepare(`UPDATE invoices SET customer_id=?, content=?, amount=?, invoice_date=?, due_date=?, status=?, updated_at=datetime('now') WHERE id=?`),
deleteInvoice: db.prepare(`DELETE FROM invoices WHERE id=?`),
updateInvoiceStatus: db.prepare(`UPDATE invoices SET status=?, updated_at=datetime('now') WHERE id=?`),
getNextInvoiceNumber: db.prepare(`SELECT COALESCE(MAX(CAST(SUBSTR(invoice_id, 5) AS INTEGER)), 0) + 1 as next_num FROM invoices`),
};
